...
**Les 76 stories du projet sont closes**, chacune avec son gate PASS.
...
| V44 | Bloc déclaré (11.3) | `blueprint/content/blocks/granit.json` — dureté 3, `tool: pickaxe`, `requiresTool: true`, `light: 7` — avec son `granit.png`, puis **redémarrer** ; `/give @s blueprint:granit` donne un objet qui montre un **cube** et non une vignette plate ; posé, il porte sa texture sur les six faces, **éclaire** autour de lui et sonne comme la pierre ; miné **à la main** c'est long et **rien ne tombe** ; miné **à la pioche** c'est nettement plus rapide et **le bloc se ramasse** ; en créatif, casser ne lâche rien ; déclarer le même nom **aussi** dans `items/` et redémarrer → le jeu démarre, l'item garde le nom, et `/blueprint content` explique **en rouge** que le bloc a été écarté |
