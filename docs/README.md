...
| [`plan-optimisation.md`](plan-optimisation.md) | Architect | Épics 13 à 19 : sûreté du fuel, index des liens, boucle de la VM, bancs |
...
| 12 | **L'éditeur à l'usage** | 12.1 → 12.5 | **En cours** — 5 gates PASS. **12.5** : la pastille de la 12.4, revue sur capture — c'était un **hexagone**. Son arrondi réutilisait celui du châssis, qui rentre d'un pixel par rangée : à un rayon de deux c'est un coin abattu, à un rayon de onze c'est exactement la **diagonale à 45°**. Elle suit désormais un cercle, et son dégradé, jusque-là simulé par deux passages superposés qui laissaient une **couture** au milieu, est peint rangée par rangée. Le défaut le plus grave n'était pourtant pas là : la pastille était dessinée **après** le châssis du nœud — ombre, liseré, fond, tous rectangulaires — dont **quatre coins dépassaient**. La 12.4 avait fait lire la même fonction au dessin, au clic et aux fils ; elle avait oublié que le châssis aussi décide d'une forme. **12.4** : les variables **comme Unreal** — une lecture devient une **pastille** sans bandeau ni titre, teintée par le **type** de la variable, et l'écriture porte la même teinte. Le risque n'était pas l'aspect mais la **géométrie** : changer la forme d'un nœud déplace ses pins, et un pin doit s'accorder entre le fil, le clic et le dessin — la divergence qui a coûté une story en 10.11. Une seule fonction rend désormais ce décalage, lue par les trois. **12.3** : déplier une catégorie renvoyait la vue **tout en haut** — sur un index d'une trentaine d'entrées, déplier la vingtième la faisait disparaître de l'écran au moment même où on l'ouvrait. La position se garde désormais exactement, et la sélection suit son **nœud** plutôt que son numéro. La barre de défilement, qui faisait **un pixel**, en fait six et **se saisit**. **12.2** : favoris et récents retirés sur demande — avec `PalettePrefs` entier, qui n'existait que pour eux. Le retrait a fait passer la couverture du client sous son seuil, ce qui a mené à `DeclaredPack.install` — livré en 11.2 **sans un seul test** — et à un défaut dedans : un pack **vide** apparaissait chez tous ceux qui n'ont jamais rien déclaré. **12.1** : le menu d'ajout s'ouvrait **tout déplié** — 184 nœuds sous une trentaine d'en-têtes dans une fenêtre de dix lignes, soit une vingtaine d'écrans pour parcourir ce qui aurait dû être une table des matières. Il s'ouvre désormais sur son **index**, comme celui d'Unreal, avec la case **« Contextuel »** qui manquait (un fil tiré filtrait sans recours) et un vrai champ de recherche. Un défaut corrigé en chemin : ma règle d'auto-dépliage **écrasait le choix de l'auteur** — replier une petite palette ne faisait rien |
| 11 | **Contenu déclaré** | 11.1 → 11.10 | **Complet** — 10 gates PASS. **11.10**, signalée en trois mots après un essai : `player/remove_item` promettait « le nombre réellement retiré » et **mentait**. Mojang traite `maxCount == 0` comme un **comptage** et rend le total possédé — or le champ de montant vaut zéro tant qu'on n'y a rien tapé. Le premier clic sur « Déposer » **créditait tout l'inventaire sans rien prendre**. **11.9** : la **banque** — un distributeur qu'on pose et qu'on clique, deux coupures de monnaie, un compte par joueur, et un retrait qui **fait l'appoint**. Elle a surtout révélé que **quatre nœuds livrés étaient irreprésentables en BScript** (`vec/split`, `pos/split`, `map/get`, `convert/to_number`) : jamais émis, donc perdus à la relecture — et depuis la 10.16 un `.bp` est écrit à **chaque enregistrement**, ce qui en faisait une perte de données répétée et silencieuse sur la garantie centrale du produit. **11.8** a comblé l'angle mort chiffré par la 11.7 — et y a trouvé **deux nœuds livrés qui levaient à chaque exécution** : `world/raycast` passait `(Entity) null` à un `ClipContext` qui fait un `requireNonNull`, et `world/raycast_entity` donnait `null` comme tireur à `ProjectileUtil`. Tous deux dans la palette et dans la référence générée, indiscernables d'un nœud sain — parce que **rien ne les avait jamais exécutés**. Un 19ᵉ gametest fait désormais tourner les **99** nœuds non purs dans un vrai serveur. **11.7**, second audit : cent quatre-vingt-quatre nœuds livrés et **aucun test n'en exécutait un seul** — les pins étant désignés par des chaînes, un pin mal nommé ne se voit qu'en partie. Résultat rassurant (rien ne lève) et désormais **gardé**. Plus un trou de méthode : **deux épics livrés n'existaient dans aucun document de planification**, et le « rapport de fin » retardait de deux épics. **11.6**, un audit à froid : le pont d'événements tient un **index** des points d'entrée depuis plusieurs stories, et les quatre chemins filtrés l'ignoraient — à 12 000 nœuds, le budget de soixante-quatre signaux d'un tick coûtait **8,5 ms** pour ne rien trouver, contre **0,15 ms** maintenant. Plus trois trous de finition, dont un **mot pour mot celui de la 10.14** : l'épic n'existait dans aucune documentation joueur, et ses quatre points de vérification commençaient tous par « écrire un JSON à la main » — un item et un bloc d'exemple sont désormais **générés** et livrés. **11.5** : ce qu'il fallait pour que tout le reste **serve**. Les quatre premières stories avaient livré des items et des blocs réels, habillés, posables — et **parfaitement inertes** : `player_use_item` ne disait pas **quel** objet avait été utilisé, ni `player_break_block` quel bloc. Un graphe savait qu'on avait cliqué, et rien d'autre. Trois événements complétés ou ajoutés (la pose d'un bloc déclaré ne peut venir que du bloc lui-même : Fabric expose la casse, pas la pose) et quatre nœuds pour reconnaître et habiller une pile. Un **18ᵉ gametest** prouve dans un vrai serveur ce que la forme des nœuds ne prouve pas : renommer rend une **copie**, sans quoi un graphe renommerait l'objet d'un joueur à distance. **11.4** : **huit touches** qu'un graphe écoute. Le graphe ne voit jamais la touche physique, seulement l'**emplacement** : l'auteur déclare « emplacement 3 », le **joueur** décide que ce sera `K`, et un conflit avec un autre mod se règle là où le jeu règle déjà tous les conflits de touches. Un code de touche aurait en outre fait cesser de fonctionner en QWERTY un script écrit en AZERTY, et aurait dit au serveur ce que le joueur tape. Un défaut trouvé en relecture : un nœud posé et jamais édité était **muet** — le point d'entrée mort, celui que ce projet a déjà payé avec `signal`. La surface d'API gagne `EVENT_INPUT`, donc `BlueprintApi` passe en **1.2.0**. **11.3** : de **vrais blocs** — on les tient, on les pose, on les mine, on les ramasse. La contrainte de la 11.1 reparaît sous un autre visage : la vitesse de minage et le butin vivent tous deux dans des données de **datapack**, donc dans une sauvegarde de monde, alors que le bloc doit être enregistré avant qu'un monde existe. Écrire un tag dans la sauvegarde du joueur a été **refusé** — le fichier y survivrait à la désinstallation du mod — et les deux règles sont portées par le bloc, avec une vitesse obtenue en **posant la question au jeu** (« que ferait l'objet tenu sur de la pierre ? »), ce qui répond juste pour les outils des mods qu'on ne connaît pas. Un défaut trouvé en relecture : un même nom dans `items/` **et** `blocks/` laissait un bloc réel qu'aucun objet ne permettait de poser. **11.2** : l'item déclaré **s'affiche enfin**, avec le PNG déposé à côté de son JSON — un pack de ressources est généré dans `resourcepacks/blueprint_content/` et activé une fois. La difficulté n'était pas le rendu mais le **voisinage** : ce code supprime des fichiers dans un dossier que le joueur possède. Trois règles en découlent — ne jamais écrire dans un dossier qu'on n'a pas créé, ne supprimer que ce qu'on a écrit, ne rien faire quand rien n'a changé — et la première n'est pas théorique : **un export du joueur a été écrasé dans cette même session**, dans un dossier que git n'indexe pas. Un défaut trouvé en relecture : l'activation automatique **reprenait au joueur une décision qu'il venait de prendre**, en recochant à chaque démarrage un pack qu'il avait décoché. **11.1** : de vrais items, réellement enregistrés dans le registre du jeu depuis `blueprint/content/items/*.json` — `/give` les connaît, une recette peut les utiliser. Trois contraintes assumées et nommées, parce qu'elles viennent de Minecraft et non du mod : les registres **gèlent** avant qu'un monde existe (donc des fichiers lus au démarrage, donc un redémarrage pour ajouter un item), le mod et les mêmes fichiers **des deux côtés** en multijoueur, et les textures par un **resource pack** (11.2). Le risque dominant n'était pas l'item manquant mais le **jeu qui ne démarre pas** : un identifiant invalide fait lever Minecraft avant l'écran titre, sans nommer le fichier fautif. Et `Map.copyOf`, qui ne préserve pas l'ordre d'insertion, a repiégé le projet exactement comme en 10.5 — ici l'ordre décide des identifiants numériques du réseau, donc un ordre instable aurait permuté les items d'un monde existant, en silence |
...
**L'épic 11 (contenu déclaré) est livré** : cinq stories. Il répond à une demande simple à
énoncer et lourde à tenir : créer de *vrais* blocs et items, pas des items vanilla
renommés. J'avais recommandé l'inverse ; la demande a été réaffirmée, et l'épic la prend
telle quelle. Ce qui rendait la chose coûteuse n'a pas disparu pour autant — c'est écrit
en tête de la story 11.1, en trois contraintes, dont celle qui décide de toute la forme de
l'épic : les registres du jeu **gèlent** avant qu'un monde soit chargé, si bien qu'un
blueprint, qui vit dans une sauvegarde, ne peut structurellement pas enregistrer un item.
Les définitions sont donc des fichiers sur le disque, lus au démarrage — et **ajouter du
contenu demande un redémarrage**.

La même contrainte est revenue trois fois sous trois visages : les registres pour les
items, les **tags** de minage et les **tables de butin** pour les blocs, tous hors
d'atteinte à l'initialisation du mod. La réponse a été la même chaque fois : porter la
règle dans le code plutôt que d'écrire dans la sauvegarde du joueur, où le fichier
survivrait à la désinstallation du mod.

Aujourd'hui : un JSON déclare un item ou un bloc, il est réellement enregistré, il porte
l'image déposée à côté, il se pose et se ramasse, une touche du joueur peut réveiller un
graphe, et le graphe sait enfin de quoi on lui parle.

**Les 85 stories du projet sont closes**, chacune avec son gate PASS.
...
| V48 | L'éditeur à l'usage (12.1 → 12.5) | `Ctrl+Espace` : le menu d'ajout s'ouvre sur son **index replié**, une trentaine d'en-têtes et non 184 nœuds ; **aucun** bloc « récents » ni « favoris » ; déplier la vingtième catégorie **ne renvoie pas la vue en haut** — l'en-tête cliqué reste sous le curseur ; la barre de défilement **se saisit** à la souris et suit ; tirer un fil puis ouvrir le menu → seuls les nœuds compatibles, et décocher **« Contextuel »** rend la liste entière. Puis, sur le graphe : une **lecture de variable** est une **capsule** — aux extrémités **rondes**, sans pointe ni coin sombre qui dépasse, sans arête au milieu — teintée selon le **type** de la variable, et deux variables de types différents se distinguent **sans lire leur nom**. La sélectionner : le halo épouse la capsule. Son fil part **du milieu** de son bord droit |
| V47 | La banque (11.9, 11.10) | cliquer **Déposer sans rien taper** → « Rien à déposer », le solde ne bouge pas et **aucune pièce ne disparaît** ; `/blueprint import banque`, `/blueprint enable blueprint:example/banque`, poser le `distributeur` et clic droit → l'écran s'ouvre, clic droit sur un autre bloc → rien ; déposer 40 pièces → le solde monte de 40 et 40 pièces partent ; retirer 250 avec 40 → **refus** ; retirer 250 avec 250 → **2 lingots et 50 pièces**, solde à 0 |
| V46 | Le contenu qui sert (11.5) | câbler la sortie **item** de « Un joueur utilise un objet » vers un message : clic droit avec un `blueprint:rubis` → il le dit, avec une pomme → `minecraft:apple` ; poser un `blueprint:granit_bleu` déclenche « Un bloc déclaré est posé », poser de la pierre → **rien**, c'est voulu et le nom du nœud le dit ; casser un bloc → la sortie **block** donne son identifiant et non de l'air ; `item/create` → `item/with_name` → `player/give_item` : l'objet arrive **renommé** et la pile d'origine ne l'est pas |
| V45 | Les touches (11.4) | Options → Commandes : **huit « Action Blueprint »**, toutes **non assignées** ; en assigner une à `K` ; poser le nœud « Une touche Blueprint est pressée » (palette → Événements → **Commandes du joueur**), laisser le numéro à **1**, câbler un message, `Ctrl+S` ; presser `K` → le message arrive ; une touche assignée à l'emplacement **2** → rien, le filtre marche ; **maintenir** `K` trois secondes → **un seul** message et non soixante ; désactiver le blueprint → la touche ne fait plus rien |
| V44 | Bloc déclaré (11.3) | `granit_bleu` est **livré** dans `blueprint/content/blocks/` — dureté 3, `tool: pickaxe`, `requiresTool: true`, `light: 7`, avec sa texture — il suffit de **redémarrer** ; `/give @s blueprint:granit_bleu` donne un objet qui montre un **cube** et non une vignette plate ; posé, il porte sa texture sur les six faces, **éclaire** autour de lui et sonne comme la pierre ; miné **à la main** c'est long et **rien ne tombe** ; miné **à la pioche** c'est nettement plus rapide et **le bloc se ramasse** ; en créatif, casser ne lâche rien ; déclarer le même nom **aussi** dans `items/` et redémarrer → le jeu démarre, l'item garde le nom, et `/blueprint content` explique **en rouge** que le bloc a été écarté |
