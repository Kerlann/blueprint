...
| ☐ V50 | **La barre glisse** (épic 21) | Le point le plus visuel de l'épic, et il se juge en **comparant**. Tout est livré : `/blueprint import jauges`, `Ctrl+S`, `/blueprint enable blueprint:jauges`, se reconnecter. Un HUD montre trois jauges. Les deux premières portent **la même valeur**, avancée par le même graphe ; seul un mot-clé les distingue. Regarder : la « Répliquée » **coule**, l'« Ordinaire » **avance par paliers d'une seconde**. Puis `/gagner` → la troisième jauge **glisse** vers sa nouvelle valeur en un dixième de seconde, assez pour que le geste se voie, assez peu pour qu'il paraisse immédiat. Se reconnecter → les jauges **s'affichent à leur valeur**, elles ne se remplissent pas depuis zéro (c'est l'instantané d'arrivée). Enfin, ouvrir n'importe quel écran **modal** lié à une variable répliquée : il glisse pareillement — c'est le même mécanisme, pas deux. |
...
| ☐ V53 | **La réplication à deux clients** (épic 21) | Le point qu'**aucun test headless ne peut couvrir**, et la frontière de sécurité de l'épic. Le support est livré : `jauges.bp` (voir V50), importé et activé sur un serveur dédié, deux clients. La jauge « Votre or » est de portée joueur et répliquée. Alice tape `/gagner` : **sa** jauge monte de 25, **celle de Bob ne bouge pas**. Bob tape : la sienne monte, celle d'Alice reste où elle était. À aucun moment l'un ne voit la valeur de l'autre — **si c'est le cas, c'est une divulgation et il faut s'arrêter là**. Les deux premières jauges, de portée monde, doivent au contraire afficher **la même chose chez les deux**, sans que personne n'ait rien demandé. Faire **reconnecter** Alice : son HUD retrouve immédiatement son or et les jauges du monde, sans attendre qu'une valeur change. Vérifier pour finir qu'un client **vanilla** peut jouer sur ce serveur sans être déconnecté. La partie serveur de ce point est automatisée par le gametest `replicatedValuesReachOnlyTheirOwner` ; ce qui reste ici est ce que seuls deux écrans peuvent montrer. |
